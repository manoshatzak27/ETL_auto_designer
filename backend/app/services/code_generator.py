...
def _generate_location_script(project) -> str:
    ...
    return (
        ...
        "import pandas as pd\n"
        "\n"
        "from etl_runtime import _info, _read_str, _write_omop_csv\n"
        "\n"
        "\n"
        "def _to_lat(val):\n"
        ...
        + "    _write_omop_csv(output_df, output_file)\n"
...
def _generate_care_site_script(project) -> str:
    ...
    return (
        ...
        "\n"
        "\n"
        "from etl_runtime import _info, build_id_lookup, _write_omop_csv\n"
        "\n"
        "\n"
        "def main():\n"
        "    # Load environmental variables\n"
        ...
        + "    _write_omop_csv(df_out, output_file)\n"
...
def _generate_provider_script(project) -> str:
    ...
    return (
        ...
        "\n"
        "\n"
        "from etl_runtime import _info, _read_str, build_id_lookup, _write_omop_csv\n"
        "\n"
        "\n"
...
def _generate_stem_table_script(project) -> str:
    ...
    inner_rows = (
        ...
        "                        start_date = None\n"
        "                        start_datetime = None\n"
        "                        # No domain inherits the visit's own date automatically anymore — an\n"
        "                        # explicit Start datetime column (if configured) takes priority; if\n"
        "                        # that's unmapped or empty for this row, the visit date is used only\n"
        "                        # when the \"Fallback to visit date\" toggle is enabled for this variable.\n"
        ...
        "                                except ValueError:\n"
        "                                    print(f'WARNING: row {_src_idx} — variable {variable!r} — start datetime {_raw_start_dt!r} does not match format {_dt_fmt!r}; start date left unset')\n"
        "                            else:\n"
        "                                if not START_DATE_FALLBACK_MAP.get(variable.lower()):\n"
        "                                    print(f'WARNING: row {_src_idx} — variable {variable!r} — start datetime column {_start_dt_col!r} is empty; start date left unset')\n"
        ...
        "                                except ValueError:\n"
        "                                    print(f'WARNING: row {_src_idx} — variable {variable!r} — visit date {_raw_visit_date!r} does not match format {date_fmt!r}; start date left unset')\n"
        "                        end_date = None\n"
        "                        end_datetime = None\n"
        ...
        "                                except ValueError:\n"
        "                                    print(f'WARNING: row {_src_idx} — variable {variable!r} — end datetime {_raw_end_dt!r} does not match format {_dt_fmt!r}')\n"
        "                            else:\n"
        "                                if not END_DATE_FALLBACK_MAP.get(variable.lower()):\n"
        "                                    print(f'WARNING: row {_src_idx} — variable {variable!r} — end datetime column {_end_dt_col!r} is empty; end date left unset')\n"
        ...
        "                        print(f'WARNING: row {_src_idx} — skipping variable {variable!r} for person {person_source_value} — {_var_exc}')\n"
        "            except Exception as e:\n"
        "                print(f'WARNING: skipping row {_src_idx} — {e}')\n"
    )
...
def _generate_domain_script(table: str) -> str:
    ...
    if table == "measurement":
        row_lines = (
            ...
            "                print(f'WARNING: dropping measurement row (person_id={person_id}, column={row.get(\"source_value\")}) — missing start_date')\n"
            "                continue\n"
            "            rows.append({\n"
            "                'measurement_id': rec_id,\n"
            "                'person_id': person_id,\n"
            "                'measurement_concept_id': _si(row.get('concept_id'), 0),\n"
            "                'measurement_date': _start_date,\n"
            ...
            "                'measurement_source_value': _ss(row.get('source_value'), 50),\n"
            "                'measurement_source_concept_id': _si(row.get('source_concept_id'), 0),\n"
            "                'unit_source_value': _ss(row.get('unit_source_value'), 50),\n"
            "                'unit_source_concept_id': None,\n"
            "                'value_source_value': _ss(row.get('value_source_value'), 50),\n"
        ...
        row_lines = (
            "            _start_date = _ss(row.get('start_date'))\n"
            "            if _start_date is None:\n"
            "                print(f'WARNING: dropping observation row (person_id={person_id}, column={row.get(\"source_value\")}) — missing start_date')\n"
            "                continue\n"
            "            rows.append({\n"
            "                'observation_id': rec_id,\n"
            "                'person_id': person_id,\n"
            "                'observation_concept_id': _si(row.get('concept_id'), 0),\n"
            "                'observation_date': _start_date,\n"
            "                'observation_datetime': _ss(row.get('start_datetime')),\n"
            "                'observation_type_concept_id': _si(row.get('type_concept_id'), 32879),\n"
            "                'value_as_number': _sf(row.get('value_as_number')),\n"
            "                'value_as_string': _ss(row.get('value_as_string'), 60),\n"
            "                'value_as_concept_id': _si(row.get('value_as_concept_id')),\n"
            ...
            "                'visit_occurrence_id': _si(row.get('visit_occurrence_id')),\n"
            "                'visit_detail_id': _si(row.get('visit_detail_id')),\n"
            "                'observation_source_value': _ss(row.get('source_value'), 50),\n"
            "                'observation_source_concept_id': _si(row.get('source_concept_id'), 0),\n"
            "                'unit_source_value': _ss(row.get('unit_source_value'), 50),\n"
            "                'qualifier_source_value': _ss(row.get('qualifier_source_value'), 50),\n"
            "                'value_source_value': _ss(row.get('value_source_value'), 50),\n"
        ...
        row_lines = (
            ...
            "                'stop_reason': _ss(row.get('stop_reason'), 20),\n"
            ...
            "                'lot_number': _ss(row.get('lot_number'), 50),\n"
            "                'provider_id': _si(row.get('provider_id')),\n"
            "                'visit_occurrence_id': _si(row.get('visit_occurrence_id')),\n"
            "                'visit_detail_id': _si(row.get('visit_detail_id')),\n"
            "                'drug_source_value': _ss(row.get('source_value'), 50),\n"
            "                'drug_source_concept_id': _si(row.get('source_concept_id'), 0),\n"
            "                'route_source_value': _ss(row.get('route_source_value'), 50),\n"
            "                'dose_unit_source_value': _ss(row.get('dose_unit_source_value'), 50),\n"
            "            })\n"
        )
        ...
        row_lines = (
            "            _start_date = _ss(row.get('start_date'))\n"
            "            if _start_date is None:\n"
            "                print(f'WARNING: dropping procedure_occurrence row (person_id={person_id}, column={row.get(\"source_value\")}) — missing start_date')\n"
            "                continue\n"
            ...
            "                'visit_occurrence_id': _si(row.get('visit_occurrence_id')),\n"
            "                'visit_detail_id': _si(row.get('visit_detail_id')),\n"
            "                'procedure_source_value': _ss(row.get('source_value'), 50),\n"
            "                'procedure_source_concept_id': _si(row.get('source_concept_id'), 0),\n"
            "                'modifier_source_value': _ss(row.get('modifier_source_value'), 50),\n"
            "            })\n"
        )
        ...
        row_lines = (
            "            _start_date = _ss(row.get('start_date'))\n"
            "            if _start_date is None:\n"
            "                print(f'WARNING: dropping condition_occurrence row (person_id={person_id}, column={row.get(\"source_value\")}) — missing start_date')\n"
            "                continue\n"
            "            _cond_status_cid = _si(row.get('condition_status_concept_id'))\n"
            "            _cond_status_sv = _ss(row.get('condition_status_source_value'), 50)\n"
            ...
            "                    _cond_status_sv = _ss(row.get('value_source_value'), 50)\n"
            "            rows.append({\n"
            "                'condition_occurrence_id': rec_id,\n"
            "                'person_id': person_id,\n"
            "                'condition_concept_id': _si(row.get('concept_id'), 0),\n"
            "                'condition_start_date': _start_date,\n"
            ...
            "                'visit_occurrence_id': _si(row.get('visit_occurrence_id')),\n"
            "                'visit_detail_id': _si(row.get('visit_detail_id')),\n"
            "                'condition_source_value': _ss(row.get('source_value'), 50),\n"
    ...
    return (
        ...
        "import pandas as pd\n"
        "\n"
        "from etl_runtime import _write_omop_csv\n"
        "\n"
